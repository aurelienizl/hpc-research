...
def launch_benchmark():
    ...
    if not command or not secret_key:
        return (
            jsonify({"status": "error", "message": "Missing command or secret_key"}),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"Failed to create task directory: {str(e)}",
                }
            ),
            500,
        )
    ...
    thread = threading.Thread(
        ...
        args=(task_id, command, task_dir, pre_cmd_exec, post_cmd_exec),
    )
    thread.start()
...
def benchmark_results(task_id):
    ...
    if task["status"] != "finished":
        return (
            jsonify({"status": "error", "message": "Benchmark not finished yet"}),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": f"Error reading result files: {str(e)}"}
            ),
            500,
        )
