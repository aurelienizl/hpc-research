...
class CmdBuilder:
    ...
    def build(self) -> dict:
        ...
        if self.benchmark.command_line:
            ...
            mpi_args = (
                self.benchmark.mpi_args.strip() if self.benchmark.mpi_args else ""
            )
            ...
            for mpi_host in self.benchmark.mpi_hosts:
                host_entries.append(
                    f'echo "{mpi_host.ip} slots={mpi_host.slots}" >> hostfile.txt'
                )
            ...
            return {"pre_cmd": pre_cmd, "command_line": main_cmd, "post_cmd": post_cmd}
